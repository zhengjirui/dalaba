package com.lechuang.dalaba.view.defineView;
...
//            }
    ...
    public String isZhengshu(String str) {
        String resulut="";
        int inxDot = str.indexOf(".");
        String tem=str.substring(inxDot+1, inxDot + 3);
        if (tem.equals("00")) {
            resulut = str.substring(0, inxDot);
            return resulut;
        }
        resulut = str;
        return resulut;
            ...
            switch (msg.what) {
                case MONEY_TYPE://金额数字的滚动
                    ...
                    if (nowMoneyNum < finalMoneyNum) {
                        ...
                        //已经达到目标的金额数字，显示最终的数字
                        if (useCommaFormat) {
                            NumberRollingView.this.setText(isZhengshu(StringUtil.addComma(formatter.format(finalMoneyNum), true)));
                        } else {
                            NumberRollingView.this.setText(isZhengshu(formatter.format(finalMoneyNum)));
                        }
                    }
                    break;
                case NUM_TYPE://普通数字滚动
                    ...
                    if (nowNum < finalNum) {
                        ...
                        //已经达到目标的数字，显示最终的内容
                        if (useCommaFormat) {
                            NumberRollingView.this.setText(isZhengshu(StringUtil.addComma(String.valueOf(finalNum), false)));
                        } else {
                            NumberRollingView.this.setText(isZhengshu(String.valueOf(finalNum)));
                        }
                    }
                    break;
            }
